...
namespace CustomFunctions
{
    ...
    public class MixingCriterion
    {
        ...
        public static double Stratification(double flow, double WSL, double volume, double length, double bottom_el, double Qnet, double Qi)
        {
            ...
            R = ConsistentMaths.Pow(velocity, 3) / (depth * (Qnet - (2.0 * Qi / (kd * depth))) * (a * g) / (p * Cp));
        ...
        public static double StratificationRisk(double velocity, double WSL, double bottom_el, double Q_swnet, double WaterTemp, double AirTemp, double RH, double WindSpeed)
        {
            ...
            double emittedLW = -emissivity_w * sigma * (ConsistentMaths.Pow(273.2 + WaterTemp, 4));
            double emissivity_a = 0.0000092 * (ConsistentMaths.Pow(273.2 + AirTemp, 2));
            double absorbedLW = emissivity_a * sigma * (1 + 0.17 * (ConsistentMaths.Pow(0, 2))) * ConsistentMaths.Pow(273.2 + AirTemp, 4) * (1 - 0.3); //CHECK THIS - 0^2. O here is the assumed cloud cover.
            double Qb = absorbedLW + emittedLW;

            //Qe - heat flux of evaporation
            double ea = RH / 100 * 610.6 * ConsistentMaths.Exp((17.27 * AirTemp) / (AirTemp + 237.3));
            double es = 610.6 * ConsistentMaths.Exp((17.27 * WaterTemp) / (WaterTemp + 237.3));
            ...
            R = ConsistentMaths.Pow(velocity, 3) / (depth * (Qnet - (2.0 * Q_swnet / (kd * depth))) * (a * g) / (p * Cp));
