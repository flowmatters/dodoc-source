...
namespace CustomFunctions
{
    ...
    public class MixingCriterion
    {
        ...
        [CustomFunction("StratificationRisk", "Calculate mixing criterion from Bormans and Webster (1997).\nInputs are flow (ML/d), water surface elevation (m AHD), storage volume (ML), length of reach (m), bottom elevation (m AHD) of reach, net shortwave radiation (W/m2), Water Temp (degrees), Air Temp (degrees), Relative Humidity (%) and Wind Speed (m/s).\nUnits must match above, not handled automatically\nDepth needs to be a \"reach average\". It is suggested to determine this using WL_NPL_m-vol_NPL_m3/area_NPL_m2\n45,000 is the treshold to disrupt stratification")]
        public static double StratificationRisk(double flow, double WSL, double volume, double length, double bottom_el, double Q_swnet, double WaterTemp, double AirTemp, double RH,double WindSpeed)
        {
